...
export function ChefCard({order}){
...
({data})=>{setMenu(data)}
  ...
  return (
    <div className="bg-transparent relative">
        <Link
          to={"/Chef/single/"+id}
          className=" border text-black bg-white flex gap-3 mt-4 p-2 items-center"
        >
          <div className={order?.made? "flex relative rounded-full  justify-center items-center min-w-16 h-16 text bg-[#37FF4B]":"flex relative rounded-full  justify-center items-center min-w-16 h-16 text bg-red-500"}>
          ...
          <div className=' text-black absolute top-1 left-20 w-20 rounded-md bg-gray-100 flex justify-start'>
            <span className='font-semibold'>ከ፡ {order?.waiter}</span>
          </div>
