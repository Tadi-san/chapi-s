...
export const CashierSingle = () => {
  const componentRef = useRef(null);
  const {id} = useParams()
  const [order, setOrder] = useState()
  const [user, setUser] = useState()
  function pay(){
    axios.post('/order/pay/'+id, {name:user?.name}).then(({data})=>{
        setOrder(data)
    })
  } 
  ...
  function unpay(){
    axios.post('/order/unpay/'+id).then(({data})=>{
        setOrder(data)
    })
  }
  ...
  return (
    <div className=" flex flex-col items-center w-full ">
      <ReactToPrint
        trigger={() => <button className="absolute border text-black top-2">Print the table</button>}
        content={() => componentRef.current}
        documentTitle="new document"
        pageStyle="print"
      />
      <div ref={componentRef} className="p-5 text-black w-4/6 px-24">
      <div className=" flex justify-between items-center">
      <Link to={'/Cashier'} className="text-black mb-4">
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
    ...
    <div className=" text-black text-2xl mb-40 mt-8 bg-gray-100 p-4">
        ...
        <div className="p-3  w-full flex flex-col items-center absolute bottom-0">
            { order?.paid == true? 
          ...
          <button onClick={unpay}
           ...
           :
            ...
            <button onClick={pay}
             className="bg-green-400 w-4/6 p-4 text-3xl mt-4 text-white rounded-full ">ክፈል</button>}
        </div>
      </div>
      </div>
    </div>
