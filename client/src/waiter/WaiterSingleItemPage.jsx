...
function deleteOrder(){
  axios.get("/order/delete/"+id).then(({data})=>{
      console.log(data)
      setRedirect(true)
  })
}
  useEffect(()=>{
axios.get('/orders/find/'+id).then(({data})=>{
setOrder(data)
    })
  },[id])
  ...
  return (
    <div className=" p-5 text-[#d3ac75]">
      <div className=" w-full flex justify-between items-center">
      <Link to={'/waiter'} className="text-[#d3ac75] mb-4">
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
      ...
      {show && (      <div className=" absolute -top-2 bg-[#222] z-10 w-56 right-4 h-32 flex flex-col items-center justify-center  rounded-xl p-2">
        ...
        <div className=" mt-3 flex justify-between w-full p-2">
          <button onClick={()=>setShow(false)}
           className="px-4 p-2 rounded-full text-white bg-[#d3ac75] ">No</button> <button onClick={deleteOrder}
      ...
      <div className="bg-[#333] p-2 text-2xl text-[#d3ac75] flex gap-2 justify-center relative mt-2">
        <span>ወንበር ቁጥር</span> <span>{order?.table}</span>
      </div>
      <div className="mt-16 flex flex-col gap-4 items-center max-h-[450px] p-2 overflow-y-scroll ">
        
        <BillLine id={id} />
        <div className=" text-[#d3ac75] text-lg mt-8 bg-gray-100 px-4">
        </div>
        <div className=" text-[#d3ac75] text-2xl mt-1 bg-gray-100 p-4">
